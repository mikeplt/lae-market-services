#!/usr/bin/env python3
"""LAE Macro Analysis – Chart.js Test Version
Identische Daten wie generate.py, Chart.js statt Plotly.
Ausgabe: outputs/macro-analysis/lae-macro-analysis-DATUM-chartjs.html

Aufruf:
  python skills/macro-analysis/generate_chartjs.py
  python skills/macro-analysis/generate_chartjs.py --api-key DEIN_FRED_KEY
"""

import os, sys, json, argparse, importlib.util, math
from pathlib import Path

# ── Daten-Funktionen aus generate.py importieren ──────────────────────────────
_spec = importlib.util.spec_from_file_location("_gen", Path(__file__).parent / "generate.py")
_gen  = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_gen)

yahoo               = _gen.yahoo
bls_batch           = _gen.bls_batch
fetch_gdp           = _gen.fetch_gdp
fred                = _gen.fred
sig                 = _gen.sig
macro_score         = _gen.macro_score
build_calendar_html = _gen.build_calendar_html
ai_interpretation   = _gen.ai_interpretation
signal_card         = _gen.signal_card
kpi_card            = _gen.kpi_card
last_val            = _gen.last_val
TODAY               = _gen.TODAY
DATE_STR            = _gen.DATE_STR
OUTPUT_DIR          = _gen.OUTPUT_DIR
